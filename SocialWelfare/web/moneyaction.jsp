...
String paymode = request.getParameter("paymode");
    ...
    int r;
        ...
        if (paymode.equals("offline")) {
            String log = "insert into offlinepay(username,amount,message) values ('" + username + "','" + amount + "','" + msg + "')";
             r = DB.connect.insertData(log);
             if(r>0)
             {    
            response.sendRedirect("donatethank.jsp");
             }
        } else if (paymode.equals("online")) {
             String log = "insert into onlinepay(username,card_no,amount,cvv,cardexpry) values ('" + username + "','" + cardnumber + "','" + amount + "','" + cvv + "','" + cardexpry + "')";
             r = DB.connect.insertData(log);
             if(r>0)
             {  
